"""
Improved Multi-Class Training Script with Two-Stage AST
Stage 1: Train for high accuracy (60 epochs)
Stage 2: Apply AST for compression (20 epochs)
Target: 95%+ accuracy with 70%+ energy savings
"""
...
CONFIG = {
    ...
    'checkpoint_dir': 'checkpoints_multiclass_improved',
    'target_activation_rate': 0.25,  # 25% activation = 75% energy savings
    'device': 'cuda' if torch.cuda.is_available() else 'cpu',
    'model_variant': 'b2',  # Use EfficientNet-B2 (better accuracy)
}
...
print(f"Stage 1: {CONFIG['stage1_epochs']} epochs (no AST, max accuracy)")
print(f"Stage 2: {CONFIG['stage2_epochs']} epochs (AST compression)")
...
# Improved augmentation for medical images
...
class AdaptiveSparseModel(nn.Module):
    ...
    def enable_ast(self):
        ...
        print("\n>>> AST ENABLED - Compression mode activated <<<\n")
...
print(f"\nTarget Activation Rate (Stage 2): {CONFIG['target_activation_rate']*100:.1f}%")

# ============================================================================
# Training Function
# ============================================================================

def train_epoch(model, loader, criterion, optimizer, device, apply_ast=False):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
metrics_df.to_csv(Path(CONFIG['checkpoint_dir']) / 'metrics_improved.csv', index=False)

print("\n" + "="*70)
print("TRAINING COMPLETE!")
print("="*70)
print(f"\nStage 1 Best Accuracy: {best_val_acc_stage1:.2f}% (no compression)")
...
print(f"  - {CONFIG['checkpoint_dir']}/metrics_improved.csv")
print("\nNext: Run visualization cells in notebook!")
